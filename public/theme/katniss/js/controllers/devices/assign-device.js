	...
	app.controller('AssignSTB',['$scope','WebService',function($scope,WebService){
		...
		var loadLco = function(){
			var http = WebService.get('assign-device/ajax_load_assign_stb_data');
		...
		$scope.setItem = function(stbs){
			console.log($scope.stb_box_id,$scope.stb_box_id);
	...
	if ($scope.stb_type_id || $scope.stb_number) {
		$scope.url = "assign-device/search_device?stb_type_id="+$scope.stb_type_id+"&smartcard_number="+$scope.stb_number;
	}
	else if ($scope.stb_type_id && $scope.stb_number) {
		$scope.url = "assign-device/search_device?stb_type_id="+$scope.stb_type_id+"&smartcard_number="+$scope.stb_number;
	}
	else{
		$scope.url = "assign-device/search_device";
	}
	if($scope.gridFistTime){
		generateKendoGird();
		$scope.gridFistTime = false;
	}else{
		reloadGridWithNewData("#grid", "assign-device/search_device?stb_type_id="+$scope.stb_type_id+"&stb_number="+$scope.stb_number);
	}
};
...
var generateKendoGird = function(data){
	$scope.mainGridOptions = {
		...
		columns: [
		{field: "", title: "",width:"100px",headerTemplate: "<input type='checkbox' ng-click='selectAllItem()' /> All", template:"<input type='checkbox' ng-checked='stb_box_id.indexOf(#=data.stb_box_id#) != -1' ng-click='setItem(#=JSON.stringify(data)#)' ng-value='#=data.stb_box_id#'/>"},
		{field: "device_number", title: "Device Number",width: "auto"},
	    ...
	    $scope.saveLcoStb = function(){
	    	...
	    	var http = WebService.post('assign-device/assign_device_to_lco',formData);
	    	http.then(function(response){
	    		var data = response.data;
		...
		var loadPermissions = function(){
			var http = WebService.get('assign-device/ajax_get_permissions');
			http.then(function(response){
				var data = response.data;
				if(data.status == 200){
