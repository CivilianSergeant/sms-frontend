...
app.controller('CreateStb',['$scope','WebService','FileUploader',function($scope,WebService,FileUploader){
	...
	$scope.saveStb = function(){
		...
		var http = WebService.post('devices/create',$scope.stb);
	...
	var generateKendoGird = function(){
		$scope.mainGridOptions = {
            ...
            columns: [
	            ...
	            {field: "", title: "Action",width: "auto",template:"<a href='"+SITE_URL+"devices/view/#=data.id#' class='btn btn-default btn-xs' data-toggle='tooltip' data-placement='left' title='View'><i class='fa fa-search'></i></a> <a ng-if='permissions.edit_permission==\"1\"' href='"+SITE_URL+"devices/edit/#=data.id#' class='btn btn-default btn-xs' data-toggle='tooltip' data-placement='left' title='Update'><i class='fa fa-pencil'></i></a>"},
            ]
        };
    };

	var loadPermissions = function(){
		var http = WebService.get('devices/ajax_get_permissions');
		http.then(function(response){
			var data = response.data;
			if(data.status == 200){
