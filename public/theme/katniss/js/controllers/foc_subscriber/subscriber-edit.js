...
app.controller('EditSubscriberProfile',['$scope','WebService','FileUploader',function($scope,WebService,FileUploader){
    ...
    $scope.setTab = function(tab){
        ...
        switch(tab){
            ...
            case 'documents':
                ...
                $scope.tabs = {profile:0,login:0,billing_address:0,documents:1,business_region:0,stb_card:0,package_assign:0,recharge:0,invoice:0,tools:0};
                break;
    ...
    $scope.updateProfile = function(){
        ...
        var http = WebService.post('foc-subscriber/update_profile',$scope.profile);
        http.then(function(response){
            var data = response.data;
            if(data.status == 400){
                $scope.warning_messages = data.warning_messages;
    ...
    $scope.isDisabledAssignPackage = function(){
        ...
        if($scope.stbCard){
            ...
            if($scope.stbCard.free_subscription_fee == 1){
                ...
                if($scope.stb_card_id == null || $scope.assigned_packages.length == 0 || $scope.charge_type == undefined){
    ...
    $scope.assignPackage = function(){
        ...
        var http = WebService.post('foc-subscriber/has_package_assigned',formData);
        http.then(function(response){
    ...
    $scope.confirmPackageItem = function(i){
        ...
        var http = WebService.post('foc-subscriber/save_assign_packages',item);
        http.then(function(response){
            var data = response.data;
    ...
    var loadProfile = function(){
        ...
        var http = WebService.get('foc-subscriber/ajax_get_profile/'+$scope.token);
        http.then(function(response){
            var data = response.data;
...
*/
    ...
    var validateForm = function(){
           ...
           if($scope.profile.contact != undefined){
                if(val != null)
                {
                    if(!val.match(pattern)){
                        $scope.warning_messages = 'Contact Number should be numeric value';
        ...
        /*$scope.$watch('profile.billing_contact',function(val){
            ...
            if($scope.billing_address != undefined){
                ...
                {
                    if(!val.match(pattern)){
                        $scope.warning_messages = 'Contact Number should be numerica value';
    ...
    $scope.sendAuthorizationRequest = function(){
        ...
        var http = WebService.post('foc-subscriber/send-authorization-request',formData);
        http.then(function(response){
            var data = response.data;
            if(data.status == 200){
                $scope.success_messages = data.success_messages;
    ...
    $scope.sendPairRequest = function(){
        ...
        var http = WebService.post('foc-subscriber/send-pair-request',formData);
        http.then(function(response){
            var data = response.data;
            if(data.status == 200){
                $scope.success_messages = data.success_messages;
    ...
    $scope.sendUnPairRequest = function(){
        ...
        var http = WebService.post('foc-subscriber/send-unpair-request',formData);
        http.then(function(response){
            var data = response.data;
            if(data.status == 200){
                $scope.success_messages = data.success_messages;
