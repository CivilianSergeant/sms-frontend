...
app.controller('PackageMigrate',['$scope','WebService',function($scope,WebService){
   ...
   /* var loadBalance = function(){
    ...
    $scope.migrate = function(item)
    {
        ...
        var http = WebService.post('foc-subscriber/ajax-get-foc-migration-amount',$scope.formData);
    	http.then(function(response){
            var data = response.data;
    ...
    {
    	var http = WebService.post('foc-subscriber/unsubscribe',$scope.formData);
    	http.then(function(response){
    		var data = response.data;
    		if(data.status == 200){
    			$scope.success_message = 'Package successfully unsubscribed';
    			window.location = SITE_URL + 'foc-subscriber/package-reassign/'+$scope.token+'/'+data.stb_card_id;
    		}else{
...
app.controller('PackageReAssign',['$scope','WebService',function($scope,WebService){
    ...
    $scope.reassignPackages = function(){
        ...
        var http = WebService.post('foc-subscriber/save_foc_reassign_packages',formData);
        http.then(function(response){
            var data = response.data;
            if(data.status == 400){
                $scope.warning_messages = data.warning_messages;
