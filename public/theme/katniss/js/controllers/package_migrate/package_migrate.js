...
app.controller('PackageMigrate',['$scope','WebService',function($scope,WebService){
    ...
    $scope.migrate = function(item)
    {
        ...
        var http = WebService.post('subscriber/ajax-get-subscriber-migration-amount',$scope.formData);
        http.then(function(response){
            var data = response.data;
    ...
    {
        $scope.closeAlert();
    	var http = WebService.post('subscriber/unsubscribe',$scope.formData);
    	http.then(function(response){
    		var data = response.data;
    		if(data.status == 200){
                $scope.loadNotification();
    			$scope.success_message = 'Package successfully unsubscribed';
    			window.location = SITE_URL + 'subscriber/package-reassign/'+$scope.token+'/'+data.stb_card_id;
    		} else {
...
app.controller('PackageReAssign',['$scope','WebService',function($scope,WebService){
    ...
    $scope.reassignPackages = function(){
        ...
        var http = WebService.post('subscriber/save_reassign_packages',formData);
        http.then(function(response){
            var data = response.data;
            ...
            if(data.status == 400){
                ...
                $("html,body").animate({scrollTop:'0px'},function(){
                    window.location = SITE_URL+'subscriber';
                });
