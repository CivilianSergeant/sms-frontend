...
app.controller('SerialProgram',['$scope','WebService',function($scope,WebService){
    ...
    $scope.confirmDelete = function(){
        $scope.closeAlert();
        var http = WebService.post('serial-contents/remove-mapping',{id:$scope.delete_hls_item,password:$scope.formData.password});
        http.then(function(response){
            var data = response.data;
            if(data.status==200){
