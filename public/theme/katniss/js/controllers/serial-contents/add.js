...
app.controller('SerialProgram',['$scope','WebService','FileUploader',function($scope,WebService,FileUploader){

    $scope.addFromFlag = 0;
    $scope.programs = [];
    $scope.permissions = {};
    $scope.verifyData = {};
    $scope.formData = {lcn:0,individual_price:0,is_active:1,is_available:1,video_language:"Bengali/Bangla"};
    $scope.formData.service_operator_id = [];
    $scope.types = []; //[{name:'CATCHUP',value:'CATCHUP'},{name:'VOD',value:'VOD'}];
    $scope.showDurationFlag = false;
    $scope.toggleTags = false;
    $scope.imageProcessing = 0;

    $scope.setToggleTags = function(){
    ...
    var loadPermissions = function(){
        var http = WebService.get('serial-contents/ajax-get-permissions');
    ...
    var generateKendoGird = function(){
        $scope.mainGridOptions = {
            dataSource: {
                transport: {
                    read: {
                        url: "serial-contents/ajax-get-serial-programs",
                        dataType: "json",
                    }
                },
                schema: {
            ...
            columns: [
                ...
                {field: "", title: "Action", width: "auto",filterable:false,template:"<a href='"+SITE_URL+"serial-contents/view/#=data.id#' class='btn btn-default btn-xs' data-toggle='tooltip' data-placement='left' title='View'><i class='fa fa-search'></i></a>"+
                ' <a ng-if="permissions.edit_permission==\'1\'" href="'+SITE_URL+'serial-contents/edit/#=data.id#" class="btn btn-default btn-xs" data-toggle="tooltip" data-placement="left" title="Edit"><i class="fa fa-pencil"></i></a> <a href="'+SITE_URL+'serial-contents/mapping/#=data.id#" data-toggle="tooltip" data-placement="left" title="Add Streamer Instance" class="btn btn-success btn-xs"><i class="fa fa-plus-circle"></i></a> # if ((data.assigned>0))  {##} else {#<a ng-if="permissions.delete_permission==\'1\'" ng-click="delete(#=data.id#)" class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="left" title="Delete"><i class="fa fa-trash"></i></a>#}#'},
            ]
        };
    ...
    $scope.confirm_delete = function()
    {
        ...
        var http = WebService.post('serial-contents/delete',$scope.verifyData);
        http.then(function(response){
            var data = response.data;
            if(data.status == 400){
    ...
    var UploaderImage = $scope.UploaderImage = new FileUploader({
        headers: {'X-Requested-With':'XMLHttpRequest'},
        url: SITE_URL+'serial-contents/upload-image'
    });


    var UploaderWatermark = $scope.UploaderWatermark = new FileUploader({
        headers: {'X-Requested-With':'XMLHttpRequest'},
        url: SITE_URL+'serial-contents/upload-water-mark'
    });
    ...
    $scope.saveIpTvProgram = function()
    {
        ...
        var dH = $("input.duration_h").val();
            dH = (dH != "")? dH : "00";
        var dM = $("input.duration_m").val();
            dM = (dM != "")? dM : "00";
        var dS = $("input.duration_s").val();
            dS = (dS != "")? dS : "00";
        ...
        var http = WebService.post('serial-contents/save-program',$scope.formData);
        http.then(function(response){
           var data = response.data;
            if(data.status == 400){
                $scope.warning_messages = data.warning_messages;
    ...
    $scope.showDuration = function(){
        ...
        var http = WebService.get('serial-contents/ajax-get-categories/'+$scope.formData.type);  
        http.then(function(response){
            var data = response.data;
            if(data.status == 200){
    ...
    $scope.isChecked = function($val){
        ...
        if($scope.formData.service_operator_id != undefined && ($val != undefined|| $val != "" || $val != null)){
    ...
    $scope.loadSubCategories = function(){
        ...
        var http = WebService.get('serial-contents/ajax-get-sub-categories/'+categoryId);
        http.then(function(response){
            var data= response.data;
