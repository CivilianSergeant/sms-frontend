...
app.controller('MappingSerialProgram',['$scope','WebService',function($scope,WebService){
    $scope.hls = [];
    $scope.operators = [];
    $scope.formData = {};
    ...
    $scope.addRow = function(){
        ...
        var domainUrl = instance.alias_domain_url+'/vod/mobile/'+contentDir+'/5/';
    ...
    var loadLco = function(){
        var http = WebService.get('serial-contents/ajax-get-lco');
    ...
    {
        var http = WebService.get('serial-contents/ajax-get-streamer-instance/'+$scope.formData.operator_id);
        http.then(function(response){
            var data = response.data;
            if(data.status == 200){
    ...
    $scope.saveMapping = function(){
        ...
        var http = WebService.post('serial-contents/save-mapping',$scope.formData);
        http.then(function(response){
            var data = response.data;
            if(data.status == 200){
                $scope.success_messages = data.success_messages;
                //window.location = SITE_URL + 'serial-contents';
            }else{
