...
app.controller('conditionalSearch',['$scope','WebService',function($scope,WebService){
	...
	var loadAllLco = function(){
		var http = WebService.get('tools-conditional-search/ajax-get-lco');
	...
	$scope.loadSubscriber = function(){
		...
		var http = WebService.get('tools-conditional-search/ajax-get-subscriber-by-lco/'+$scope.lco_id);
		http.then(function(response){
			var data = response.data;
			if(data.status == 200){
	...
	$scope.loadPairings = function(){
		$scope.pairings = [];
		$scope.pairing_id = null;
		var http = WebService.get('tools-conditional-search/ajax-get-pairing/'+$scope.subscriber_id);
		http.then(function(response){
			var data = response.data;
			if(data.status == 200){
	...
	{
		var http = WebService.get('tools-conditional-search/ajax-get-regions');
		http.then(function(response){
			var data = response.data;
			if(data.status == 200)
			{
	...
	$scope.sendConditionalSearch = function(){
		...
		var http = WebService.post('tools-conditional-search/process-search',formData);
		http.then(function(response){
			var data = response.data;
			if(data.status == 400){
