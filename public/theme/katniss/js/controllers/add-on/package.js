...
app.controller('package',['$scope','WebService',function($scope,WebService){
	...
	$scope.showForm = function()
	{
        if($scope.permissions.create_permission=="1") {
            ...
            $scope.warning_messages = "Sorry! You don't have permission to create Add-on package";
        }
	}
    ...
    $scope.savePackage = function(){
    	...
    	var http = WebService.post('add-on-package/save',formData);
    ...
    var loadPrograms = function(){
    	var http = WebService.get('add-on-package/ajax_load_programs');
    	http.then(function(response){
			var data = response.data;
			if(data.status == 200){
    ...
    var generateKendoGird = function(){
        $scope.mainGridOptions = {
            dataSource: {
                transport: {
                    read: {
                        url: "add-on-package/ajax_load_package",
                        dataType: "json",
                    }
                },
                schema: {
            ...
            columns: [
                ...
                {field: "", title: "Action", width: "auto",filterable:false,template:"<a href='"+SITE_URL+"add-on-package/view/#=data.token#' class='btn btn-default btn-xs' data-toggle='tooltip' data-placement='left' title='View'><i class='fa fa-search'></i></a>"+
                ' <a ng-if="permissions.edit_permission==\'1\'" href="'+SITE_URL+'add-on-package/edit/#=data.token#" class="btn btn-default btn-xs" data-toggle="tooltip" data-placement="left" title="Edit"><i class="fa fa-pencil"></i></a> # if ((data.assigned>0) || (data.id == 65535 || data.id == 65534)) {##} else {#<a ng-if="permissions.delete_permission==\'1\'" ng-click="delete(#=data.id#)" class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="left" title="Delete"><i class="fa fa-trash"></i></a>#}#'},
            ]
        };
    ...
    {
        var location = SITE_URL + 'add-on-package/delete/'+$scope.delete_item;
    ...
    var loadPermissions = function(){
        var http = WebService.get('add-on-package/ajax_get_permissions');
        http.then(function(response){
            var data = response.data;
            if(data.status == 200){
...
app.controller('editPackage',['$scope','WebService','$sce',function($scope,WebService,$sce){
    ...
    $scope.savePackage = function(){
        ...
        var http = WebService.post('add-on-package/save/'+token,formData);
        http.then(function(response){
            var data = response.data;
            ...
            if(data.status == 400){
                ...
                $("htm,body").animate({scrollTop:'0px'},function(){
                    window.location = SITE_URL + 'add-on-package';
                });
    ...
    var loadPackage = function(){
        var http = WebService.get('add-on-package/ajax_load_package_programs/'+token);
        http.then(function(response){
            var data = response.data;
            if(data.status == 200){
                var pkg = data.pkg;
    ...
    var generateKendoGird = function(){
        $scope.mainGridOptions = {
            dataSource: {
                transport: {
                    read: {
                        url: SITE_URL+"add-on-package/ajax_get_assigned_package_list/"+$scope.token,
                        dataType: "json",
                    }
                },
                schema: {
