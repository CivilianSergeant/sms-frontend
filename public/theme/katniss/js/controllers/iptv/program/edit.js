...
app.controller('IptvProgram',['$scope','WebService','FileUploader',function($scope,WebService,FileUploader){
    ...
    var loadProgram  = function(){
        var http = WebService.get('channels/ajax-get-program/'+programId);
        http.then(function(response){
            var data = response.data;
    ...
    UploaderImage.onCompleteItem = function(fileItem,response){
        ...
        /*$("#mobilePoster").attr('src',BASE_URL+response.image.poster_url_mobile);
        $("#webPoster").attr('src',BASE_URL+response.image.poster_url_web);
        $("#stbPoster").attr('src',BASE_URL+response.image.poster_url_stb);*/
    };
    ...
    UploaderChannelLogo.onCompleteItem = function(fileItem,response){
        ...
        if(response.status == 400){
            ...
            if(response.status == 200){
                /*$("#posterSTB").attr('src',BASE_URL+response.image);*/
                $scope.imageProcessing = 0;
                window.location.reload();
            }
        }
    ...
    $scope.updateIpTvProgram = function(){
        ...
        $scope.formData.duration = ($scope.formData.individual_price > 0)? dH+":"+dM+":"+dS : '00:00:00';
        
        console.log('h')
        var http = WebService.post('channels/update-program',$scope.formData);
        http.then(function(response){
            var data = response.data;
            if(data.status == 400){
                ...
                $("html,body").animate({scrollTop:'0px'},500,'swing',function(){
                   // window.location = SITE_URL+'programs';
                });
