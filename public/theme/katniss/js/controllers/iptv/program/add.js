...
app.controller('IptvProgram', ['$scope', 'WebService', 'FileUploader', function ($scope, WebService, FileUploader) {
        ...
        var generateKendoGird = function () {
            $scope.mainGridOptions = {
                dataSource: {
                    transport: {
                        read: {
                            url: "channels/ajax-get-programs",
                            dataType: "json",
                        }
                    },
                    schema: {
                ...
                columns: [
                    ...
                    {field: "mapped", title: "Epg Mapped", width: "200px", filterable: false},
                    {field: "lcn", title: "LCN", width: "auto", filterable: false},
                    ...
                    {field: "", title: "Action", width: "auto", filterable: false, template: "<a href='" + SITE_URL + "channels/view/#=data.id#' class='btn btn-default btn-xs' data-toggle='tooltip' data-placement='left' title='View'><i class='fa fa-search'></i></a>" +
                                ' <a ng-if="permissions.edit_permission==\'1\'" href="' + SITE_URL + 'channels/edit/#=data.id#" class="btn btn-default btn-xs" data-toggle="tooltip" data-placement="left" title="Edit"><i class="fa fa-pencil"></i></a> <a href="' + SITE_URL + 'channels/mapping/#=data.id#" class="btn btn-success btn-xs" data-toggle="tooltip" data-placement="left" title="Add Streamer Instance"><i class="fa fa-plus-circle"></i></a> # if ((data.assigned>0))  {##} else {#<a ng-if="permissions.delete_permission==\'1\'" ng-click="delete(#=data.id#)" class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="left" title="Delete"><i class="fa fa-trash"></i></a>#}#'},
                ]
            };
        ...
        $scope.confirm_delete = function ()
        {
            ...
            var http = WebService.post('channels/delete', $scope.verifyData);
            http.then(function (response) {
                var data = response.data;
                if (data.status == 400) {
        ...
        var UploaderImage = $scope.UploaderImage = new FileUploader({
            headers: {'X-Requested-With': 'XMLHttpRequest'},
            url: SITE_URL + 'channels/upload-image'
        });


        var UploaderChannelLogo = $scope.UploaderChannelLogo = new FileUploader({
            headers: {'X-Requested-With': 'XMLHttpRequest'},
            url: SITE_URL + 'channels/upload-channel-logo'
        });

        var UploaderWatermark = $scope.UploaderWatermark = new FileUploader({
            headers: {'X-Requested-With': 'XMLHttpRequest'},
            url: SITE_URL + 'channels/upload-water-mark'
        });
        ...
        UploaderChannelLogo.onCompleteItem = function (fileItem, response) {
        ...
        $scope.saveIpTvProgram = function ()
        {
            ...
            var http = WebService.post('channels/save-program', $scope.formData);
            http.then(function (response) {
                var data = response.data;
                if (data.status == 400) {
                    $scope.warning_messages = data.warning_messages;
                    ...
                    UploaderImage.onBeforeUploadItem = function (item) {
                        ...
                        item.formData.push({id: data.id, form_type: 0,
                            ...
                            mobile_thumb: $scope.formData.mobile_thumb,
                    ...
                    // upload Logo Mobile URL
                    UploaderChannelLogo.onBeforeUploadItem = function (item) {
                        UploaderChannelLogo.progress = 0;
                        $scope.fileUploadPhotoProgress = 0;
                        item.formData.push({id: data.id, form_type: 0});
                        $scope.imageProcessing = 1;
                        $("html,body").animate({scrollTop: '0px'});
                    };
                    UploaderChannelLogo.uploadAll();
