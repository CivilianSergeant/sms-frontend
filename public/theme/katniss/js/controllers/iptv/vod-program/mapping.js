...
app.controller('MappingProgram',['$scope','WebService',function($scope,WebService){
    ...
    var loadLco = function(){
        var http = WebService.get('vod-programs/ajax-get-lco');
    ...
    {
        var http = WebService.get('vod-programs/ajax-get-streamer-instance/'+$scope.formData.operator_id);
        http.then(function(response){
            var data = response.data;
            if(data.status == 200){
    ...
    $scope.saveMapping = function(){
        ...
        var http = WebService.post('vod-programs/save-mapping',$scope.formData);
        http.then(function(response){
            var data = response.data;
            if(data.status == 200){
                $scope.success_messages = data.success_messages;
                //window.location = SITE_URL + 'vod-programs';
            }else{
