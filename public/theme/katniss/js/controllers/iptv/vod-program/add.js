...
app.controller('IptvProgram',['$scope','WebService','FileUploader',function($scope,WebService,FileUploader){
    ...
    $scope.permissions = {};
    $scope.verifyData = {};
    $scope.formData = {type:'VOD',lcn:0,individual_price:0,is_active:1,is_available:1,video_language:"Bengali/Bangla"};
    ...
    var loadPermissions = function(){
        var http = WebService.get('vod-programs/ajax-get-permissions');
    ...
    var generateKendoGird = function(){
        $scope.mainGridOptions = {
            dataSource: {
                transport: {
                    read: {
                        url: "vod-programs/ajax-get-programs",
                        dataType: "json",
                    }
                },
                schema: {
            ...
            columns: [
                ...
                {field: "", title: "Action", width: "auto",filterable:false,template:"<a href='"+SITE_URL+"vod-programs/view/#=data.id#' class='btn btn-default btn-xs' data-toggle='tooltip' data-placement='left' title='View'><i class='fa fa-search'></i></a>"+
                ' <a ng-if="permissions.edit_permission==\'1\'" href="'+SITE_URL+'vod-programs/edit/#=data.id#" class="btn btn-default btn-xs" data-toggle="tooltip" data-placement="left" title="Edit"><i class="fa fa-pencil"></i></a> <a href="'+SITE_URL+'vod-programs/mapping/#=data.id#" class="btn btn-success btn-xs" data-toggle="tooltip" data-placement="left" title="Add Streamer Instance"><i class="fa fa-plus-circle"></i></a> # if ((data.assigned>0))  {##} else {#<a ng-if="permissions.delete_permission==\'1\'" ng-click="delete(#=data.id#)" class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="left" title="Delete"><i class="fa fa-trash"></i></a>#}#'},
            ]
        };
    ...
    $scope.confirm_delete = function()
    {
        ...
        var http = WebService.post('vod-programs/delete',$scope.verifyData);
        http.then(function(response){
            var data = response.data;
            if(data.status == 400){
    ...
    $scope.saveIpTvProgram = function()
    {
        ...
        var http = WebService.post('vod-programs/save-program',$scope.formData);
        http.then(function(response){
           var data = response.data;
            if(data.status == 400){
                $scope.warning_messages = data.warning_messages;
