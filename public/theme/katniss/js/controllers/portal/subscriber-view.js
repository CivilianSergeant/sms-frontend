...
app.controller('EditSubscriberProfile',['$scope','WebService','FileUploader',function($scope,WebService,FileUploader){
    ...
    var loadBalance = function(){
        var http = WebService.post('profile/ajax-get-balance',{token:$scope.token});
        http.then(function(response){
            var data = response.data;
            if(data.status == 200){
    ...
    var loadBusinessRegion = function(){
        var http = WebService.get('profile/ajax-load-region');
        http.then(function(response){
            var data = response.data;
            $scope.regions = data;
...
*/
    ...
    var loadLocations = function(type,model){
        ...
        var http = WebService.post('profile/ajax-get-location/'+type,formData);
        http.then(function(response){
            var data = response.data;
            if(type == 'divisions'){
