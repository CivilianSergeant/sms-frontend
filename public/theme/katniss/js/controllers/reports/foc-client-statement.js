...
app.controller('ClientStatement',['$scope','WebService',function($scope,WebService){
    ...
    $scope.loadSubscriber = function(){
        ...
        var http = WebService.get('foc-client-statement/ajax-get-subscriber-by-lco/'+$scope.user_id);
    ...
    $scope.loadPairings = function(){
        $scope.cards = [];
        $scope.stbcards = [];
        var http = WebService.get('foc-client-statement/ajax-get-pairings/'+$scope.formData.subscriber_id);
        http.then(function(response){
            var data = response.data;
            if(data.status == 200){
    ...
    $scope.getStatements = function(){
        ...
        $scope.totalCredit  = 0;
        $scope.totalDebit   = 0;
        var http = WebService.post('foc-client-statement/ajax-get-statements',$scope.formData);
        http.then(function(response){
            var data = response.data;
            if(data.status == 200){
