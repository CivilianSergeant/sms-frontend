...
class Conditional_limited_model extends MY_Model
{
	protected $table_name="conditional_limited_logs";
	...
	public function get_all_logs($limit=0,$offset=0,$count=false,$subscriber_id,$stb,$smart_card,$status=null)
	{
		...
		condition_return_code,start_time,end_time,is_stoped,stb_id,smart_card_id,(end_time < NOW()) as expired';
		...
		if(!empty($smart_card))
			$this->db->where('smart_card_id', $smart_card);
