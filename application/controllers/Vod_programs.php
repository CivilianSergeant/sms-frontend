 ...
 */
class Vod_programs extends BaseController
{
    ...
    public function index()
    {
        ...
        $this->theme->set_title('Vod Content')
            ->add_style('kendo/css/kendo.common-bootstrap.min.css')
            ->add_style('kendo/css/kendo.bootstrap.min.css')
            ->add_style('component.css')
            ->add_script('controllers/iptv/vod-program/add.js');
       ...
       // $data['unassigned_programs'] = $this->program->count_unassigned_program();
        ...
        $data['categories'] = $this->Iptv_category->get_vod_categories($parent_id);
        ...
        $this->theme->set_view('iptv/vod-program/program',$data,true);
    }
    ...
    public function ajax_get_permissions()
    {
        if($this->input->is_ajax_request()){
            if($this->role_type == self::ADMIN){
                ...
                $permissions = $this->menus->has_permission($role,1,'vod-programs',$this->user_type);
            }
    ...
    public function ajax_get_programs()
    {
        if($this->input->is_ajax_request()){
            ...
            $programs = $this->Iptv_program->get_vod_programs($id,$take,$skip,$filter,$sort);
            $total = $this->Iptv_program->count_vod_programs($id,$filter);
    ...
    public function save_mapping()
    {
        if($this->input->is_ajax_request()){
            ...
            if(!$program->has_attributes()){
                echo json_encode(array('status'=>400,'warning_messages'=>'Sorry! VoD Content not found to map HLS link'));
                exit;
            }
            ...
            $title = 'VoD Content and Instance successfully mapped';
            $msg = 'Congratulations! Total ['.$counter.'] HLS link mapped with VoD Content ['.$program->get_attribute('program_name').'], instance ['.$instance->get_attribute('instance_name').']';
    ...
    public function remove_mapping()
    {
        if($this->input->is_ajax_request()){
            ...
            if($this->role_type == self::STAFF) {
                $permission = $this->menus->has_delete_permission($this->role_id, 1, 'vod-programs', $this->user_type);
                if (!$permission) {
    ...
    public function upload_image(){
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if($this->role_type == "staff") {
                if ($form_type) {
                    $permission = $this->menus->has_edit_permission($this->role_id, 1, 'vod-programs', $this->user_type);
                    if (!$permission) {
                        echo json_encode(array('status' => 400, 'warning_messages' => "Sorry! You don't have edit permission"));
                        exit;
                    }
                } else {
                    $permission = $this->menus->has_create_permission($this->role_id, 1, 'vod-programs', $this->user_type);
                    if (!$permission) {
                ...
                if(preg_match('/(png)/',$_FILES['file']['type'])){
                    ...
                    try {
                        ...
                        if(move_uploaded_file($tmp_uploaded_file,$main_photo_path)){
                            ...
                            /*$photo_data['logo_web_url']      = $program->get_attribute('logo_web_url');
                            $photo_data['logo_stb_url']      = $program->get_attribute('logo_stb_url');
                            $photo_data['logo_mobile_url']   = $program->get_attribute('logo_mobile_url');
                    ...
                    if ($form_type) {
                        $this->set_notification("VoD Content logo Uploaded", "Logo of VoD Content [{$program->get_attribute('program_name')}] has been uploaded");
                    }
                    if($warningMsg != null){
                    ...
                    echo json_encode(array('status'=>400,'redirect'=>site_url('vod-programs/edit/'.$id),'warning_messages'=>'Logo File Type is not valid'));
                }

            } else {
                echo json_encode(array('status'=>400,'warning_messages'=>'No VoD Content found to attach logo'));
            }

        } else {
            $this->session->set_flashdata('warning_messages','Sorry! Direct access not allowed');
            redirect('vod-programs');
        }
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($program->has_attributes()) {
                if(preg_match('/(png)/',$_FILES['file']['type'])){
                    ...
                    //$this->mso_profile->save($photo_data,$profile->get_attribute('id'));
                    if ($form_type) {
                        $this->set_notification("VoD Content Logo(mobile) Uploaded", "Mobile Logo of VoD Content [{$program->get_attribute('program_name')}] has been uploaded");
                    }
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($program->has_attributes()) {
                if(preg_match('/(png)/',$_FILES['file']['type'])){
                    ...
                    //$this->mso_profile->save($photo_data,$profile->get_attribute('id'));
                    if ($form_type) {
                        $this->set_notification("VoD Content Poster Uploaded", "Poster of VoD Content [{$program->get_attribute('program_name')}] has been uploaded");
                    }
                ...
                echo json_encode(array('status'=>400,'warning_messages'=>'No VoD Content found to attach poster'));
            }
    ...
    public function save_program()
    {
        if($this->input->is_ajax_request()){
            ...
            if($this->form_validation->run() == FALSE){
                ...
                $programName = $this->Iptv_program->is_name_available($this->input->post('program_name'),'VOD');
                ...
                // set notification
                $title = "VoD Content created.";
                $msg   = "A New VoD Content {$saveData['program_name']} has been created";
                $this->set_notification($title,$msg);
                ...
                echo json_encode(array(
                    'status'=>200,
                    'id'=>$saved,
                    'success_messages'=>'VoD Content '. $saveData['program_name'].' successfully saved')
                );
            }
    ...
    public function edit($id)
    {
        ...
        if(!$iptvProgram->has_attributes()){
            $this->session->set_flashdata('warning_messages','VoD Content not found');
            redirect('vod-programs');
        }

        $this->theme->set_title('Edit VoD Content')
            ->add_style('kendo/css/kendo.common-bootstrap.min.css')
            ->add_style('kendo/css/kendo.bootstrap.min.css')
            ->add_style('component.css')
            ->add_script('controllers/iptv/vod-program/edit.js');
        ...
        $this->theme->set_view('iptv/vod-program/edit-program',$data,true);
    ...
    public function update_program()
    {
        if($this->input->is_ajax_request()){
            ...
            // set notification
            $title = "VoD Content Updated.";
            $msg   = 'VoD Content '.$programName.' has been successfully updated';
            $this->set_notification($title,$msg);
    ...
    public function view($id)
    {
        ...
        $this->theme->set_title('View VoD Content')
            ->add_style('kendo/css/kendo.common-bootstrap.min.css')
            ->add_style('kendo/css/kendo.bootstrap.min.css')
            ->add_style('component.css')
            ->add_script('controllers/iptv/vod-program/view.js');
        ...
        $data['content_aggregator_types'] = $this->content_aggregator_type->get_all();
        $id = ($this->role_type == self::STAFF)? $this->parent_id : $this->user_id;
        $data['categories'] = $this->Iptv_category->get_vod_categories($id);
        ...
        $this->theme->set_view('iptv/vod-program/view-program',$data,true);
    ...
    public function delete()
    {
        ...
        if($program_check != Null){
            $this->session->set_flashdata('warning_messages','VoD Content is already used');
            redirect('vod-programs');
        }else{
            ...
            // set notification
            $title = "VoD Content Deleted.";
            $msg   = 'VoD Content Information '.$program_name.' has been deleted';
            $this->set_notification($title,$msg);
    ...
    public function mapping($id)
    {
        $iptvProgram = $this->Iptv_program->find_by_id($id);
        if(!$iptvProgram->has_attributes()){
            $this->session->set_flashdata('warning_messages','Sorry! VoD Content not found, Please check is content exist');
            redirect('vod-programs');
        }

        $this->theme->set_title('Mapping VoD Contents')
            ->add_style('kendo/css/kendo.common-bootstrap.min.css')
            ->add_style('kendo/css/kendo.bootstrap.min.css')
            ->add_style('component.css')
            ->add_script('controllers/iptv/vod-program/mapping.js');
        ...
        $this->theme->set_view('iptv/vod-program/mapping-program',$data,true);
    }
