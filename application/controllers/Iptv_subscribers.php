 ...
 */
class Iptv_subscribers extends BaseController
{
    ...
    public function index()
    {
        $this->theme->set_title('Subscriber Creation - Application')
            ...
            ->add_script('controllers/iptv/subscriber/subscriber.js');
        ...
        $this->theme->set_view('iptv/subscriber_profile/subscriber_profile', $data, true);
    }
    ...
    public function ajax_get_profiles()
    {
        ...
        $all_subscribers = $this->subscriber_profile->get_all_iptv_subscribers($id,$take,$skip,$filter,$sort);
    ...
    public function edit($token)
    {
        ...
        if ($profile->has_attributes()) {
            $this->theme->set_title('IPTV Subscriber Edit - Application')
                ->add_style('kendo/css/kendo.common-material.min.css')
                ->add_style('kendo/css/kendo.material.min.css')
                ->add_style('component.css')
                ->add_script('controllers/iptv/subscriber/subscriber-edit.js');
            $data['user_info'] = $this->user_session;

            $data['left_sidebar'] = $this->theme->set_sidebar('left', $data);
            $data['token'] = $token;
            $data['theme'] = $this->theme->get_image_path();
            $this->theme->set_view('iptv/subscriber_profile/edit_subscriber_profile', $data, true);
        } else {
    ...
    public function view($token)
    {
        ...
        if ($profile->has_attributes()) {
            $this->theme->set_title('IPTV Subscriber View - Application')
                ->add_style('kendo/css/kendo.common-material.min.css')
                ->add_style('kendo/css/kendo.material.min.css')
                ->add_style('component.css')
                ->add_script('controllers/iptv/subscriber/subscriber-view.js');
            $data['user_info'] = $this->user_session;

            $data['left_sidebar'] = $this->theme->set_sidebar('left', $data);
            $data['token'] = $token;
            $data['theme'] = $this->theme->get_image_path();
            $this->theme->set_view('iptv/subscriber_profile/view_subscriber_profile', $data, true);
        } else {
            $this->session->set_flashdata('warning_messages','Sorry! No data found');
            redirect('iptv-subscribers');
        }
