...
class Settop_box extends BaseController 
{
	...
	*/
	public function index()
	{
		...
		$this->theme->set_view('settop_box/settop_box',$data,true);
	}
	...
	public function create_stb()
	{
		if($this->role_type == 'staff') {
			$permission = $this->menus->has_create_permission($this->role_id, 1, 'set-top-box', $this->user_type);
			if (!$permission) {
		...
		$this->form_validation->set_rules('external_card_number', 'External Number','required|max_length[16]|min_length[16]|is_unique[set_top_boxes.external_card_number]');
		...
		if ($this->form_validation->run() == FALSE) {
			if ($this->input->is_ajax_request()) {
				...
				redirect('set-top-box');
			}
		} else {

				$save_stb_data = array(
					// 'internal_card_number'   => $this->input->post('internal_card_number'),
					'internal_card_number' => 0,
					'external_card_number'  => $this->input->post('external_card_number'),
					'stb_card_provider'  => $this->input->post('stb_card_provider'),
					'price'  => $this->input->post('price')
					);

				$this->set_top_box->creat_stb($save_stb_data);
				$this->set_notification("New Set-top-box Added","Set-top-box {$save_stb_data['external_card_number']} has been added successfully");
				echo json_encode(array('status'=>200, 'success_messages'=>'Set-Top Box ' . $save_stb_data['internal_card_number'] . ' created successfully'));
		}
	}
	...
	{
		if($this->role_type == 'staff') {
			$permission = $this->menus->has_edit_permission($this->role_id, 1, 'set-top-box', $this->user_type);
			if (!$permission) {
		...
		$data['left_sidebar'] = $this->theme->set_sidebar('left',$data);
		$data['stb_providers'] = $this->stb_provider->get_all_stb_providers();
		$data['stbs'] = $this->set_top_box->stb_by_id($id);
		$data['theme'] = $this->theme->get_image_path();
		$this->theme->set_view('settop_box/edit_settop_box',$data,true);
	}
	...
	public function update_action()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			$this->session->set_flashdata('warning_messages',validation_errors());
			redirect('update-set-top-box/' . $this->input->post('id'));
		} else {
			...
			//test($stb_data);
			$update = $this->set_top_box->save($stb_data, $stb_data['id']);
			if ($update) {
				$this->set_notification("Set-top-box Info updated","Set-top-box {$stb_data['external_card_number']} information has been changed");
				$this->session->set_flashdata('success', 'STB ' . $stb_data['external_card_number'] . ' Updated Successfully');
				redirect('set-top-box');
			}
		}
	}
	...
	public function view_stb($id)
	{
		...
		$data['stbs'] = $this->set_top_box->stb_by_id($id);
		$data['theme'] = $this->theme->get_image_path();
		$this->theme->set_view('settop_box/view_stb',$data,true);
	}
	...
	public function ajax_load_stb()
	{
		...
		$all_stb = $this->set_top_box->get_all_stb($id, array($take, $skip,$filter,$sort));
	...
	public function export_stb()
	{
		...
		if($this->input->post()){
			...
			$data = array(
			    array('External Card Number','Stb Provider','Price'),
			    array('Use Text Format','Keep it Same','Keep it Same'),
			    array('',$card_provider,$price)
			);
			$file_name = 'public/downloads/exports/stb-template.xlsx';
			...
			try{
						...
						foreach($data as $i => $value){
							if($i>=2){
								...
								$save_card_data[] = array(
									...
									'stb_card_provider'    => $stb_card_provider,
									'price' => $price,
				...
				if(!empty($save_card_data)){
					foreach($save_card_data as $data){
						$this->set_top_box->save($data);
					}
					$this->set_notification("STB Imported","STB numbers Successfully imported");
					echo json_encode(array('status'=>200,'success_messages'=>'STB numbers Successfully imported'));
					exit;
				}
