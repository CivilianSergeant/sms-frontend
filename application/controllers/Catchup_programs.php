 ...
 */
class Catchup_programs extends BaseController
{
    ...
    public function index()
    {
        ...
        $this->theme->set_title('Catchup Content')
            ->add_style('kendo/css/kendo.common-bootstrap.min.css')
            ->add_style('kendo/css/kendo.bootstrap.min.css')
            ->add_style('component.css')
            ->add_script('controllers/iptv/catchup-program/add.js');
        ...
        $data['categories'] = $this->Iptv_category->get_catchup_categories($parent_id);
        ...
        $this->theme->set_view('iptv/catchup-program/program',$data,true);
    }
    ...
    public function ajax_get_permissions()
    {
        if($this->input->is_ajax_request()){
            if($this->role_type == self::ADMIN){
                ...
                $permissions = $this->menus->has_permission($role,1,'catchup-programs',$this->user_type);
            }
    ...
    public function ajax_get_programs()
    {
        if($this->input->is_ajax_request()){
            ...
            $id = ($this->role_type == self::STAFF)? $this->parent_id : $this->user_id;
            $programs = $this->Iptv_program->get_catchup_programs($id,$take,$skip,$filter,$sort);

            $total = $this->Iptv_program->count_catchup_programs($id,$filter);
    ...
    public function save_mapping()
    {
        if($this->input->is_ajax_request()){
            ...
            if(!$program->has_attributes()){
                echo json_encode(array('status'=>400,'warning_messages'=>'Sorry! Catchup Content not found to map HLS link'));
                exit;
            }
            ...
            $title = 'Catchup Content and Instance successfully mapped';
    ...
    public function remove_mapping()
    {
        if($this->input->is_ajax_request()){
            if($this->role_type == self::STAFF) {
                $permission = $this->menus->has_delete_permission($this->role_id, 1, 'catchup-programs', $this->user_type);
                if (!$permission) {
    ...
    public function upload_image(){
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if($this->role_type == "staff") {
                if ($form_type) {
                    $permission = $this->menus->has_edit_permission($this->role_id, 1, 'catchup-programs', $this->user_type);
                    if (!$permission) {
                        echo json_encode(array('status' => 400, 'warning_messages' => "Sorry! You don't have edit permission"));
                        exit;
                    }
                } else {
                    $permission = $this->menus->has_create_permission($this->role_id, 1, 'catchup-programs', $this->user_type);
                    if (!$permission) {
            ...
            if ($program->has_attributes()) {
                if(preg_match('/(png)/',$_FILES['file']['type'])){
                    ...
                    if ($form_type) {
                        $this->set_notification("Catchup Content logo Uploaded", "Logo of Catchup Content [{$program->get_attribute('program_name')}] has been uploaded");
                    }
                    if($warningMsg != null){
                    ...
                    echo json_encode(array('status'=>400,'redirect'=>site_url('catchup-programs/edit/'.$id),'warning_messages'=>'Logo File Type is not valid'));
                }

            } else {
                echo json_encode(array('status'=>400,'warning_messages'=>'No Catchup Content found to attach logo'));
            }

        } else {
            $this->session->set_flashdata('warning_messages','Sorry! Direct access not allowed');
            redirect('catchup-programs');
        }
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($program->has_attributes()) {
                if(preg_match('/(png)/',$_FILES['file']['type'])){
                    ...
                    //$this->mso_profile->save($photo_data,$profile->get_attribute('id'));
                    if ($form_type) {
                        $this->set_notification("Catchup Content Logo(mobile) Uploaded", "Mobile Logo of Catchup Content [{$program->get_attribute('program_name')}] has been uploaded");
                    }
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($program->has_attributes()) {
                if(preg_match('/(png)/',$_FILES['file']['type'])){
                    ...
                    //$this->mso_profile->save($photo_data,$profile->get_attribute('id'));
                    if ($form_type) {
                        $this->set_notification("Catchup Content Poster Uploaded", "Poster of Catchup Content [{$program->get_attribute('program_name')}] has been uploaded");
                    }
                ...
                echo json_encode(array('status'=>400,'warning_messages'=>'No Catchup Content found to attach poster'));
            }
    ...
    public function save_program()
    {
        if($this->input->is_ajax_request()){
            ...
            if($this->form_validation->run() == FALSE){
                ...
                $programName = $this->Iptv_program->is_name_available($this->input->post('program_name'),'CATCHUP');
                ...
                // set notification
                $title = " Catchup Content created.";
                $msg   = "A New Catchup Content {$saveData['program_name']} has been created";
                $this->set_notification($title,$msg);
    ...
    public function edit($id)
    {
        ...
        if(!$iptvProgram->has_attributes()){
            $this->session->set_flashdata('warning_messages','Catchup Content not found');
            redirect('catchup-programs');
        }

        $this->theme->set_title('Edit Catchup Content')
            ->add_style('kendo/css/kendo.common-bootstrap.min.css')
            ->add_style('kendo/css/kendo.bootstrap.min.css')
            ->add_style('component.css')
            ->add_script('controllers/iptv/catchup-program/edit.js');
        ...
        $this->theme->set_view('iptv/catchup-program/edit-program',$data,true);
    ...
    public function view($id)
    {
        ...
        $this->theme->set_title('View Catchup Content')
            ->add_style('kendo/css/kendo.common-bootstrap.min.css')
            ->add_style('kendo/css/kendo.bootstrap.min.css')
            ->add_style('component.css')
            ->add_script('controllers/iptv/catchup-program/view.js');
        ...
        $this->theme->set_view('iptv/catchup-program/view-program',$data,true);
    }
    ...
    public function delete()
    {
        ...
        if($program_check != Null){
            ...
            // set notification
            $title = "Catchup Content Deleted.";
            $msg   = 'Content Information '.$program_name.' has been deleted';
            $this->set_notification($title,$msg);
    ...
    public function mapping($id)
    {
        ...
        $this->theme->set_title('Mapping Catchup Content')
            ->add_style('kendo/css/kendo.common-bootstrap.min.css')
            ->add_style('kendo/css/kendo.bootstrap.min.css')
            ->add_style('component.css')
            ->add_script('controllers/iptv/catchup-program/mapping.js');
        ...
        $this->theme->set_view('iptv/catchup-program/mapping-program',$data,true);
    }
