...
<style>
	.blank-img{
        ...
        .thumbnails{
		width:100px;height:60px;margin:0px 70px;
	}
</style>
		...
		<div class="row" >
			...
			<div class="col-md-12">
				<div class="panel-heading">
					<div class="col-md-12">
						<h4 class="widgettitle">View Channel [{{formData.program_name}}]
							...
							<a href="<?php echo site_url('channels/edit/'.$programId); ?>" id="buttoncancel" class="btn btn-success btn-sm pull-right" style="margin-right:10px;"><i class="fa fa-pencil"></i> Edit </a>
						</h4>
					</div>
				...
				<div class="col-md-12">
					<form class="form-horizontal" ng-submit="updateIpTvProgram()">
						...
						<div class="form-group">
							...
							<div class="col-md-4">
								<input type="text" class="form-control" readonly ng-model="formData.content_dir"/>
							</div>
						</div>
						<div class="form-group">
						...
						<hr/>
						<div class="form-group">
							<label class="control-label col-md-3">Mobile Channel Logo </label>
							<?php if(!empty($program['channel_logo']) && file_exists($program['channel_logo'])){ ?>
								<div class="col-md-8">
									<img class="thumbnails" ng-src="<?php echo base_url(); ?>{{formData.channel_logo}}"/>
									<span><?php
										list($width, $height, $type, $attr) = getimagesize($program['channel_logo']);
										echo $width.'x'.$height.' Pixel, Size: '.number_format(filesize($program['channel_logo'])/1024,2) .' KB';
										?>
									</span>
								</div>
							<?php }else{ ?>
						...
						<div class="form-group">
							...
							<div class="col-md-8">
								<img style="width:100px;height:57px;margin:0px 70px;" ng-src="<?php echo base_url(); ?>{{formData.water_mark_url}}"/>
								<span><?php
									list($width, $height, $type, $attr) = getimagesize($program['water_mark_url']);
									echo $width.'x'.$height.' Pixel, Size: '.number_format(filesize($program['water_mark_url'])/1024,2) .' KB';
									?></span>
							</div>
							<?php }else{ ?>
								<div class="col-md-3">
						...
						<div class="form-group">
							<div class="col-md-3 col-md-offset-3">
								<div class="checkbox">
									<label>
										<input type="checkbox" disabled ng-model="formData.age_restriction" ng-checked="formData.age_restriction=='1'" ng-true-value="'1'" ng-false-value="'0'"/>
						...
						<!--<div class="form-group">
							...
							<div class="col-md-2">
								<input type="number" readonly class="form-control" ng-model="formData.duration" required="required"/>
							</div>
						</div>-->
						<!--<div class="form-group">
						...
						<div class="form-group">
							<label class="control-label col-md-3">Language <span class="text-danger">*</span></label>
							<div class="col-md-2">
								<select class="form-control" disabled ng-model="formData.video_language">
							...
							<?php if(!empty($streamer_instances)){ ?>
									...
									<?php foreach($streamer_instances as $i=> $si){ ?>
										<tr >
											<td><?php echo ($i+1); ?></td>
											<td width="250"><?php
												echo (strlen($si->hls_url_stb)<=30) ? '<a data-toggle="tooltip"  title="'.$si->hls_url_stb.'" href="javascript:void(0);">'.$si->hls_url_stb.'</a>' : '<a data-toggle="tooltip"  title="'.$si->hls_url_stb.'" href="javascript:void(0);">'.substr($si->hls_url_stb,0,30).'...</a>';
												?>
											</td>
											<td width="250"><?php
												echo (strlen($si->hls_url_web)<=30) ? '<a data-toggle="tooltip"  title="'.$si->hls_url_web.'" href="javascript:void(0);">'.$si->hls_url_web.'</a>' : '<a data-toggle="tooltip" title="'.$si->hls_url_web.'" href="javascript:void(0);">'.substr($si->hls_url_web,0,30).'...</a>';
												?>
											</td>
											<td width="250"><?php
												echo (strlen($si->hls_url_mobile)<=30) ? '<a data-toggle="tooltip"  title="'.$si->hls_url_mobile.'" href="javascript:void(0);">'.$si->hls_url_mobile.'</a>' : '<a data-toggle="tooltip" title="'.$si->hls_url_mobile.'" href="javascript:void(0);">'.substr($si->hls_url_mobile,0,30).'...</a>';
												?>
											</td>
